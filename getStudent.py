...
from facPrescenceController import getFacultyPrescenceState, getAllPrescenceData, isStudentAllowedToEnter
            ...
            if students_list.status_code == 200:
                try:
                    if isStudentAllowedToEnter(str(uid).zfill(10)):
                        ...
                        le_time = str(datetime.datetime.now().time().replace(microsecond=0))
                        is_student_on_time = isBludNotLate(le_time,fac_all_data['time_in'])
                ...
                if fac_prescence == 1:
                    ...
                    try:
                        requests.post('https://www.pilocksystem.live/api/inst/' + str(fac_all_data['uid']).zfill(10), timeout=2)
                    except:
                        pass
                    return 200
                else:
